...
async function ensureNode(onProgress: ProgressCallback): Promise<void> {
  ...
  onProgress("No package manager found, installing Node.js via nvm...");

  const script = `${EXTRA_PATH}; curl -fsSL https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.3/install.sh | bash && export NVM_DIR="$HOME/.nvm" && . "$NVM_DIR/nvm.sh" && nvm install --lts`;
  ...
  return new Promise((resolve, reject) => {
    command.on("error", (err) => reject(new Error(`Node.js install failed: ${err}`)));
    command.on("close", (data) => {
      if (data.code === 0) {
        onProgress("Node.js installed successfully");
        resolve();
      } else {
        reject(new Error(`Node.js install exited with code ${data.code}`));
      }
    });
